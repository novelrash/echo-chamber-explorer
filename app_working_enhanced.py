#!/usr/bin/env python3
"""
Echo Chamber Explorer - Bias Analysis Web Interface with AI Enhancement
"""
...
# Try enhanced analyzer first, then LangChain, then fallback to original
try:
    from analyzers.enhanced_bias_analyzer import EnhancedBiasAnalyzer
    analyzer = EnhancedBiasAnalyzer()
    AI_ENHANCED = True
    print("✅ Enhanced bias analyzer loaded (improved conservative detection)")
except Exception as e:
    try:
        from analyzers.langchain_enhanced_analyzer import LangChainEnhancedAnalyzer
        analyzer = LangChainEnhancedAnalyzer()
        AI_ENHANCED = True
        print("✅ LangChain AI agents loaded")
    except Exception as e2:
        from analyzers.integrated_bias_analyzer import IntegratedBiasAnalyzer
        analyzer = IntegratedBiasAnalyzer()
        AI_ENHANCED = False
        print(f"⚠️ Using original analyzer: {e}")
...
def analyze():
    ...
    if request.method == 'POST':
        ...
        # Perform bias analysis (enhanced, AI, or original)
        ...
        ''', (
            article_id,
            results.get('bias_score', 0.0),
            results.get('sentiment_polarity', 0.0),
            results.get('sentiment_subjectivity', 0.0),
            results.get('emotional_density', 0.0),
            results.get('certainty_ratio', 0.5),
            results.get('citation_count', 0)
        ))
